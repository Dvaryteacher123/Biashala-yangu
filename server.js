...
async function requireActiveClaim(req, res, next) {
  // Anza kusoma kwenye headers kwanza (muhimu sana kwa ajili ya multipart/form-data na fetch headers)
  let userId = req.headers['x-user-id'] || req.headers['X-User-Id'] || req.body?.userId || req.body?.user_id || req.query?.userId || req.query?.user_id;
...
// ============================================================
// API: ADD BUSINESS (Requires Active Claim)
// ============================================================
...
app.delete('/api/businesses/:id', requireActiveClaim, async (req, res) => {
  ...
  try {
    ...
    if (existing.image_url) {
      try {
        const pathStr = existing.image_url.split('/').slice(-2).join('/');
        filesToDelete.push(pathStr);
      } catch (e) {}
    }
    if (existing.video_url) {
      try {
        const pathStr = existing.video_url.split('/').slice(-2).join('/');
        filesToDelete.push(pathStr);
      } catch (e) {}
    }
    if (existing.category_image) {
      try {
        const pathStr = existing.category_image.split('/').slice(-2).join('/');
        filesToDelete.push(pathStr);
      } catch (e) {}
    }
...
// ============================================================
// API: UPLOAD FILE (Multer) - IMEREKEBISHWA
// ============================================================
app.post('/api/upload-file', requireActiveClaim, upload.single('file'), async (req, res) => {
  const userId = req.body.userId || req.body.user_id || req.headers['x-user-id'] || req.headers['X-User-Id'];
  const file = req.file;
...
app.post('/api/register-business', requireActiveClaim, upload.fields([
  ...
  try {
    ...
    const finalUserId = user_id || req.headers['x-user-id'] || req.headers['X-User-Id'];
    ...
    if (files && files.image) {
      ...
      const imgName = `${finalUserId}/${Date.now()}-img${imgExt}`;
    ...
    if (files && files.video) {
      ...
      const vidName = `${finalUserId}/${Date.now()}-vid${vidExt}`;
